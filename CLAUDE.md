...
| Route | Description |
|---|---|
| `/` | Landing page: `HeroSection`, which renders the sticky `Navbar`, a minimal hero (headline + embedded `PropertyListings` search), and the **"How it works" section** (`id="how-it-works"`) — this is `ComparisonDemo`, an auto-playing animated walkthrough (cursor bookmarks 3 homes → table fills → best-value highlights cascade; loops, Replay button, respects `prefers-reduced-motion`). There is no longer a separate step strip; the demo *is* the "How it works" section. |
