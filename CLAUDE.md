...
- `POST /api/checkout` — **requires auth**; creates a Stripe **subscription** Checkout session ($19/mo, inline `price_data` so no dashboard product is needed), `client_reference_id` = the Supabase user id, returns `{ url }`. The `/pricing` "Upgrade to Pro" button POSTs here and `window.location.href`s to the returned Stripe URL. Success → `/dashboard?upgraded=1`, cancel → `/pricing`.
